...
const path = {
  ...
  watch: {
    ...
    image: "src/assets/images/**/*.+(jpg|png|jpeg|gif|ico|webp)",
...
gulp.task("cssLibs", function () {
  return gulp
    .src([
      ...
      "./src/styles/css-libs/bootstrap-grid.min.css",
    ]) //поик файлов
    .pipe(plumber()) //обработка ошибок
...
gulp.task("otf2ttf", function () {
  return gulp
    ...
    .pipe(dest(path.src.fonts)); //перенос файла в папку
});
...
gulp.task("watch", function () {
  isDevelopment = false;
  gulp.watch([path.watch.html], { delay: 1200 }, gulp.parallel("html"));
  gulp.watch([path.watch.scss], { delay: 1200 }, gulp.parallel("scss"));
  gulp.watch([path.watch.css], { delay: 1200 }, gulp.parallel("cssLibs"));
  gulp.watch([path.watch.js], { delay: 1200 }, gulp.parallel("js"));
  gulp.watch([path.watch.image], { delay: 1200 }, gulp.parallel("image"));
  gulp.watch(
    [path.watch.svg],
    { delay: 1200 },
    gulp.parallel("buildSvgSprite")
  );
});
